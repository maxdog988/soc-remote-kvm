...
#ifndef LIBVNCSERVER_HAVE_POLEG
...
#ifdef LIBVNCSERVER_HAVE_POLEG
static rfbBool sendHextiles16HW(rfbClientPtr cl, int rx, int ry, int rw, int rh);
#endif
                           ...
                           int h)
{
    ...
    case 16:
#ifdef LIBVNCSERVER_HAVE_POLEG
        return sendHextiles16HW(cl, x, y, w, h);
#else
...
#ifdef LIBVNCSERVER_HAVE_POLEG
static rfbBool
sendHextiles16HW(rfbClientPtr cl, int rx, int ry, int rw, int rh) {
    ...
    if ((err = ioctl(nurfb->hextile_fd, GETED , &cmd)) < 0) {
        rfbLog("X11vnc:GETED failed:%d\n",err);
        return FALSE;
    }

    copy_addr = nurfb->raw_ece_addr;
